...
cd "$ROOT"
mkdir -p "$LOG_DIR"
LOG_FILE=${VLM_LOG_FILE:-$LOG_DIR/$VLM_RUN_NAME.log}

"$PYTHON" -u scripts/train_value.py \
  --data_dir "$PROGRESS_DATA_DIR" \
  --checkpoint_dir "$VLM_CKPT_DIR" \
  --batch_size ${VLM_BATCH_SIZE:-32} \
  --num_train_steps ${VLM_TRAIN_STEPS:-10000} \
  --load_pretrained \
  --wandb_mode "$WANDB_MODE" \
  --wandb_project pistar \
  --wandb_run_name "$VLM_RUN_NAME" \
  --log_interval ${VLM_LOG_INTERVAL:-100} \
  --save_interval ${VLM_SAVE_INTERVAL:-5000} \
  --val_interval 0 \
  --peak_lr ${VLM_PEAK_LR:-2.5e-5} \
  --decay_lr ${VLM_DECAY_LR:-2.5e-6} \
  --warmup_steps ${VLM_WARMUP_STEPS:-1000} \
  --freeze_mode ${VLM_FREEZE_MODE:-all_backbones} \
  --num_workers ${VLM_NUM_WORKERS:-0} \
  --fsdp_devices ${VLM_FSDP_DEVICES:-8} \
  2>&1 | tee "$LOG_FILE"
