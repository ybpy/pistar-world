"""
Pistar LeRobot 数据处理流程脚本，从 LeRobot 格式读取并计算 reward/value/adv/epsilon

处理流程：
1. Pass 1: 从 LeRobot 数据集加载所有数据，计算 reward 和 value
2. Pass 2: 计算 advantages，按 task 统计 advantages 并计算 epsilon (70% 分位数)
3. Pass 3: 基于 epsilon 计算 adv_ind，写入新的 LeRobot 数据集
...
Usage:
# 从 LeRobot 数据集计算 adv 和 adv_ind
python examples/libero/pistar_lerobot_data_processing.py \
...
python -u examples/libero/pistar_lerobot_data_processing.py ... 
"""
...
def main(
    ...
    *,
    ...
    # ========================================================================
    print("\n" + "=" * 80)
    print("📊 Pass 1: 加载 LeRobot 数据集并计算 reward/value")
    print("=" * 80)
    ...
    for episode_idx in range(input_dataset.num_episodes):
        ...
        for step_idx in range(episode_length):
            ...
            if step_idx == 0:
                episode_data['task'] = frame.get('task', f'episode_{episode_idx}')
    ...
    print(f"\n✅ Pass 1 complete: {len(all_episodes_data)} episodes loaded")
    ...
    # ========================================================================
    print("\n" + "=" * 80)
    print("💾 Pass 3: 计算 adv_ind 并写入新数据集")
    print("=" * 80)
